...
class Export(object):
    ...
    def new_Export(self, module_name):
        ...
        # 订单 导出
        if iee.is_element_exist(*self.new_export_id2):
        ...
        # 轨迹导出
        if iee.is_element_exist(*self.new_export_id3):
        ...
        if iee.is_element_exist(*self.expor_name_btn_client_export):
            ...
            for id in range(number):
                ...
                self.driver.find_element(*self.close_order_window).click()
        # 费用申请导出
        if iee.is_element_exist(*self.expor_name_btn_cost_export):
            ...
            for id in range(number):
                ...
                self.driver.find_element(*self.close_order_window).click()
        # 费用报销导出
        if iee.is_element_exist(*self.expor_name_btn_restitution_export):
